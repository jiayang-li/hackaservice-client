...
import { IonicPage, NavController, Platform, AlertController, normalizeURL } from 'ionic-angular';

@Injectable()
export class BarcodeProvider {
  private requestResultCodes = "false";
  constructor(private alertCtrl: AlertController, public events: Events, private platform: Platform, private barcodeScanner: BarcodeScanner) {
  ...
  public basicScan() { 
    this.barcodeScanner.scan().then(result => {
      //TODO: Confirm 1Z + 9 Character tracking numbers (Confirm 9 numerical / H + 8 numerical)
      console.log('result.txt: ' + result.text);
      console.log('next: ' + result.text.substring(0, 2));
      if(result.text.substring(0, 2) == "1Z" || (result.text.length == 9 && (Number.isInteger(Number(result.text)) || 
      (result.text.substring(0, 1) == "H") && Number.isInteger(Number(result.text.substring(1)))))){
          var url = "https://wwwapps.ups.com/WebTracking/track?track=yes&trackNums=";
          url += result.text;
     ...
     }).catch(err => { console.log('error on barcode scanning: '+ err); });
  }
}
